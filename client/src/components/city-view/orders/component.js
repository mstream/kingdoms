...
import { CityOrdersScheduledAttackItemComponent } from './scheduled-attack-item';
import type { ScheduledAttackOrderInfo } from '../../../state/modules/selectors/types';
import { CityOrdersScheduledAttackInfoComponent } from './scheduled-attack-info';
...
export const Component = ({
    ...
    const orderRowComponents = Object.keys(orderInfosById).map(
        (orderId: string) => {
            ...
            const onClick = (event) => {
                selectCityViewOrdersTab({ orderId });
            };
    ...
    return (
        <div
            ...
            role="tabpanel"
        >
            <div className="flex flex-col w-1/3 m-1 shadow-inner bg-gray-900-alpha-50">
                {orderRowComponents}
            </div>
            <div className="flex flex-row w-1/3 m-1 shadow-inner bg-gray-900-alpha-50">
                {infoComponent}
            </div>
        </div>
    );
