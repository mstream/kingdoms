...
import { CityOrdersScheduledAttackItemComponent } from './index';
...
describe('CityOrdersScheduledAttackItemComponent', () => {
    test('order scheduled in future', async () => {
        ...
        expect(queryByText('Attack Citytwo')).toBeInTheDocument();
        expect(queryByText('in 1h 40m')).toBeInTheDocument();
    });

    test('order not executed on time', async () => {
        const state: ClientState = {
            ...emptyClientState,
            commonState: {
                ...emptyCommonState,
        ...
        expect(queryByText('Attack Citytwo')).toBeInTheDocument();
        expect(queryByText('delayed by 30m')).toBeInTheDocument();
    });
});
