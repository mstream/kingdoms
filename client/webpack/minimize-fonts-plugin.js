    ...
    return Object.keys(compilation.assets)
        .reduce(
            (unicodeGlyphs, assetName) => {
                ...
                const unicodeMatches = matches.map(match => {
...
const minimize = (compilation, done) => {
    const fonts = Object.keys(compilation.assets)
        ...
        .sort(
            (asset1, asset2) => asset1.buffer.length - asset1.buffer.length,
        );
    ...
    const minimizedTtfFontPromises = ttfFonts.map(
        (font) => {
            return new Promise(
                (resolve, reject) => {
                    const fontmin = new Fontmin().use(Fontmin.glyph({ text: unicodeGlyphs.join(' ') }));
            ...
            (minimizedTtfFonts) => {
                minimizedTtfFonts.forEach(
                    (minimizedTtfFont, index) => {
                        ...
                        compilation.assets[minimizedTtfFont.asset] = new RawSource(minimizedTtfFont.buffer);
                        compilation.assets[woff2Font.asset] = new RawSource(ttf2woff2(minimizedTtfFont.buffer));
                    },
                );
                done();
            },
