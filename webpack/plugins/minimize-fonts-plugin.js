                ...
                const unicodeMatches = matches
                    .map(
                        (
                            ...
                            const unicodeMatch = match.match(
                                UNICODE_REGEX,
                            );
                            return unicodeMatch
                                ? String.fromCharCode(
...
const minimize = (
    ...
    const fonts = Object.keys(
        ...
        .map(
            (
                ...
                return {
                    ...
                    buffer: compilation.assets[ asset ]._value,
                };
        ...
        .sort(
            (
                ...
                return asset1.buffer.length - asset2.buffer.length;

            },
        );

    const ttfFonts = fonts.filter(
        (
            font,
        ) => {

            return font.asset.endsWith(
                `ttf`,
            );

        },
    );
    const woff2Fonts = fonts.filter(
        (
            font,
        ) => {
    ...
    const unicodeGlyphs = findUnicodeGlyphs(
        compilation,
    );

    const minimizedTtfFontPromises = ttfFonts.map(
        (
            font,
        ) => {
    ...
    Promise.all(
        ...
        .then(
            (
                ...
                minimizedTtfFonts.forEach(
                    (
                        minimizedTtfFont, index,
                    ) => {
